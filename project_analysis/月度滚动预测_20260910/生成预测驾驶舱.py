# -*- coding: utf-8 -*-
"""预测驾驶舱 v4：平台体系 + 参考看板交互语言（sparkline KPI / ECharts 原生交互）
砍：翻转卡、全局口径白条、口径速览、修订说明条、通俗方法说明——纯数据看板。
"""
import sys
import json
import pandas as pd
...
fused = pd.read_csv(OUT + r'\融合测试_公司口径.csv')
...
# KPI sparkline 数据
ape = [round(abs(r['实际'] - r['combo_volxasp']) / max(r['实际'], 1) * 100, 1) for _, r in fused.iterrows()]
y26 = [v for m, v in zip(hist['月'], hist万) if m >= '2026-01']

DATA = {
    'months': FUT, 'base': [round(float(x)) for x in comp['基准万']], 'unbiased': [round(float(x)) for x in comp['无偏万']],
    'histMonths': list(hist['月']), 'histVal': [round(float(x)) for x in hist万],
    ...
    'spark': {
        'fy': [round(x) for x in y26] + [round(float(x)) for x in comp['基准万'][:4]],
        'm6': [round(float(x)) for x in comp['基准万'][:6]],
        'm12': [round(float(x)) for x in comp['基准万']],
        'wape': ape,
        'dir': [int(x) for x in r3['命中']],
    },
    'kpis': [
        {'icon': 'fa-coins', 'label': '2026 全年（1-8月实际+9-12月）', 'v': '{:,.0f} 万'.format(S4), 's': '无偏 {:,.0f} 万（{:.2f} 亿）'.format(S4F, S4F / 1e4), 'c': '#2563EB', 'k': 'fy'},
        {'icon': 'fa-calendar-days', 'label': '未来 6 个月（26-09 ~ 27-02）', 'v': '{:,.0f} 万'.format(S6), 's': '无偏 {:,.0f} 万'.format(S6F), 'c': '#0EA5E9', 'k': 'm6'},
        {'icon': 'fa-calendar-check', 'label': '未来 12 个月（26-09 ~ 27-08）', 'v': '{:,.0f} 万'.format(S12), 's': '无偏 {:,.0f} 万'.format(S12F), 'c': '#8B5CF6', 'k': 'm12'},
        {'icon': 'fa-bullseye', 'label': '月度平均误差', 'v': '6.9%', 's': '无偏校正后 · 校正前 7.7%', 'c': '#F59E0B', 'k': 'wape'},
        {'icon': 'fa-compass', 'label': '方向准确率', 'v': '74%', 's': '涨跌命中 14/19 月 · 目标 65%', 'c': '#10B981', 'k': 'dir'},
    ],
}
...
.kc .value{font-size:20px;font-weight:700;font-variant-numeric:tabular-nums;margin:4px 0 2px}
.kc .sub2{font-size:11px;color:var(--text-tertiary,#94A3B8)}
.kc .spark{position:absolute;left:0;right:0;bottom:0;height:34px;pointer-events:none;opacity:.9}
...
H.append('<div class="tab-bar"><button class="tab-btn active" data-tab="P">月度滚动预测</button>'
         '<span style="margin-left:auto;font-size:11px;color:var(--text-tertiary,#94A3B8);padding:10px 14px">数据截至 2026-08 ｜ 生成 2026-09-16 ｜ 本面将并入主看板</span></div>')

H.append('<div class="tab-content active" id="tabP">')

# KPI 区（sparkline 模式，无翻转无 hint）
H.append('<div class="kpi-bar">')
for k in DATA['kpis']:
    H.append('<div class="kc" style="position:relative;padding-bottom:38px">'
             '<div class="icon-wrap" style="color:%s"><i class="fa-solid %s"></i></div>'
             '<div class="label">%s</div><div class="value">%s</div><div class="sub2">%s</div>'
             '<div class="spark" id="sp_%s"></div></div>' % (k['c'], k['icon'], k['label'], k['v'], k['s'], k['k']))
H.append('</div>')

# 图表区
H.append('<div class="cb"><h3><i class="fa-solid fa-chart-line" style="color:var(--primary)"></i> 80 个月历史 + 12 个月预测延展（万元）'
         '<span class="st">本月修订：2027-01 由 11,129 下修至 10,226 万（长历史春节标定）</span></h3><div id="cHist" style="width:100%;height:420px"></div></div>')
H.append('<div class="c2">'
         '<div class="cb"><h3><i class="fa-solid fa-chart-column" style="color:var(--primary)"></i> 分月预测（万元）<span class="st">图例点击切换基准/无偏；橙=春节规则月</span></h3><div id="cMonth" style="width:100%;height:320px"></div></div>'
...
# 产品线明细表
H.append('<div class="cb"><h3><i class="fa-solid fa-table-list" style="color:var(--primary)"></i> 产品线明细（万元）'
         '<span class="st">A档1条+B档4条覆盖约90%销售额；C档12条看结构不引数字</span></h3>'
...
tooltip:{trigger:'axis',backgroundColor:'rgba(255,255,255,0.96)',borderColor:'rgba(148,163,184,0.3)',borderWidth:1,
textStyle:{color:'#1E293B',fontSize:12},extraCssText:'box-shadow:0 8px 32px rgba(31,38,135,0.12);border-radius:10px;'},
...
function spark(id,color,data){const el=document.getElementById(id);if(!el)return;
const c=echarts.init(el,null,{renderer:'canvas'});
c.setOption({grid:{top:2,bottom:2,left:0,right:0},xAxis:{type:'category',show:false,data:data.map((_,i)=>i)},
yAxis:{type:'value',show:false,min:v=>Math.min(0,v.min*0.95)},
series:[{type:'line',data:data,smooth:true,symbol:'none',lineStyle:{width:1.8,color:color},
areaStyle:{color:{type:'linear',x:0,y:0,x2:0,y2:1,colorStops:[{offset:0,color:color+'55'},{offset:1,color:color+'00'}]}}}]});
charts.push(c);}
D.kpis.forEach(k=>spark('sp_'+k.k,k.c,D.spark[k.k]));
// 历史+预测
...
// 分月（双系列，图例切换）
mk('cMonth',{...baseOpt,legend:{data:['基准','无偏'],top:4},
xAxis:{type:'category',data:D.months.map(m=>m.slice(2).replace('-','/')),axisLabel:{color:SUB,rotate:38}},
yAxis:{type:'value',axisLabel:{color:SUB},splitLine:{lineStyle:{color:'#e2e8f0'}}},
series:[{name:'基准',type:'bar',data:D.base.map((v,i)=>({value:v,itemStyle:{color:D.months[i]>='2027-01'&&D.months[i]<='2027-02'?ORANGE:BLUE}})),barWidth:'34%',itemStyle:{borderRadius:[3,3,0,0]}},
...
with open(DEST, 'w', encoding='utf-8') as f:
    f.write('\n'.join(H))
print('已生成 v4:', DEST, '(%.0f KB)' % (os.path.getsize(DEST) / 1024))
