...
【使用方法】
  1. 双击运行（选择数据文件）
  2. 或命令行: python run_v2.8.py "数据文件路径.xlsx"
...
【输出文件】output_v2.8_年月.xlsx
  产品快照表: 每个产品一行，完整诊断字段（含策略建议、数据质量标记）
  预警清单: 自动筛选高风险产品
  画像分布: 各画像产品数统计
  趋势预测汇总: 未来3月销量预测（ETS+节假日调整，含逐月置信区间）
  历史画像追踪: 各产品12个时间点的画像变迁轨迹
  客户RFM分群: 客户价值分层与流失预警
  产品关联分析: 客户月度组合采购分析
  使用说明: 字段解释与术语表
  另输出3个HTML可视化图表（桑基图、画像分布图、风险分布图）
...
【v2.8 新增功能】
  ...
  - 字段重命名：增长率→近12月增长率, 当前毛利率→近12月毛利率, 趋势斜率→毛利率趋势斜率
  - 新增当月毛利率%、近12月销售额、前12月销售额列
  - 预测列迁移至独立sheet，使用说明移至最后
...
【v2.7 新增功能】
  ...
  - 订单频次趋势：客户采购意愿变化监测
  - RFM客户分群：客户价值分层与流失预警
  - 产品关联分析：按客户+月份聚合的购物篮分析，挖掘客户月度组合采购模式

【v2.6 更新说明】
  - 内部测试版，未正式发布
  - 测试了7因子评分模型的可行性（后回退至6因子）
...
【v2.3 更新说明】
  - 内部迭代版，未正式发布
  - 尝试7因子模型（含价格弹性因子，后因数据覆盖不足回退）

【v2.2 更新说明】
  - 引入5因子风险评分模型（毛利率斜率/客户集中度/CV/增速衰减/历史对照）
  - 新增增速变化二阶指标（百分点制）
  - 新增月度毛利率时间序列追踪
  - 特情说明系统初版（增长率窗口、客户集中度预警）
"""
...
def validate_weights(weights, thr):
    ...
    w_keys = ["毛利率趋势斜率", "订货波动性(CV)", "增速衰减", "自比健康度", "ASP趋势"]
...
def calc_slope(y_values, thr=None):
    """对 y_values 做线性回归，返回斜率（比率/月）。数据点不足时返回 0.0。"""
...
def generate_contextual_strategy(portrait, row):
    """根据画像和实际数据动态生成策略建议，避免与营收-毛利综合判断相悖。
    ...
    返回:
        str: 上下文感知的策略建议
    """
    margin_yoy = row.get("毛利率同比变化%", 0) if row.get("毛利率同比变化%") is not None else 0
...
def classify_9grid_full(momentum_full, health_full, row=None):
    ...
    参数:
        ...
        row: 可选，产品数据行（dict-like），用于生成上下文感知的策略建议
    返回:
        (画像名称, 管理层摘要, 通用策略建议)
    """
    portrait_map = {
...
def ets_forecast(monthly_qty, periods=3, seasonal_periods=0):
    ...
    参数:
        monthly_qty: list[float], 月度销量序列（按时间升序）
        periods: int, 预测月数，默认3
        seasonal_periods: int, 季节性周期长度，0=自动检测

    返回:
        (forecast, direction, pred_intervals, model_info)
        forecast: list[float] 预测值
        direction: str 趋势方向 ("上升"/"平稳"/"下降")
        pred_intervals: list[(lower, upper)] 逐月置信区间
        model_info: dict 模型选择信息
    """
    ...
    try:
        ...
        try:
            ...
            for z_val, label in [(1.28, 80), (1.96, 95)]:
                ...
                lower = [max(0, round(f - half, 0)) for f in forecast]
                upper = [max(0, round(f + half, 0)) for f in forecast]
                pred_intervals[label] = (lower, upper)
        except Exception:
...
def risk_asp(asp_slope, margin_slope, thr):
    ...
    - ASP轻度下降 且 毛利率同步恶化 → 中高风险（50分）
    - ASP明显下降（≤-0.5%/月）但毛利率稳定 → 中低风险（20分）
    - ASP明显下降 且 毛利率同步恶化 → 高风险（50分）
    ...
    margin_t = float(thr.get("斜率_中分阈值%/月", -0.3)) / 100
    ...
    if asp_slope >= t_low:
        ...
        if margin_slope is not None and margin_slope <= margin_t:
            return 50  # ASP微降但毛利率也在下降，升级风险
        return 20
    elif asp_slope > t_high:
        # ASP明显下降：检查毛利率是否同步恶化
        if margin_slope is not None and margin_slope > margin_t:
            return 20  # ASP下降但毛利率稳定，降级风险
        return 50
    else:
        return default
...
def calc_price_elasticity(monthly_prices, monthly_qtys, thr):
    """计算价格弹性系数。稳健方法：百分比变化中位数 + IQR过滤，结果裁剪。"""
...
def product_association_analysis(df, name_col, date_col, cust_col, thr):
    """产品关联分析（按客户+月份聚合的购物篮分析）。

    将同一客户在同一个月内购买的所有产品视为一个"购物篮"，
    相比按订单号聚合，能发现更多有意义的跨产品关联关系。
    ...
    min_support = float(thr.get("关联_最小支持度", 0.02))
    min_confidence = float(thr.get("关联_最小置信度", 0.15))
...
def risk_self_health(health_pct, thr):
    ...
    # 兼容小数格式（0~1范围 → 转为百分比）
    pct = health_pct * 100 if health_pct < 2 else health_pct
...
def generate_specific_note(row, thr):
    ...
    检测项包括：增长率窗口不足、增速减速、客户集中度过高、
    零销量、负毛利、增长率截断、增速衰减上限移除、策略一致性矛盾等。
    """
    notes = []
    ...
    cust_warn_line = float(thr.get("特情_客户集中预警值", 0.50))
    ...
    # v2.9: 自比健康度极低
    sh_extreme = float(thr.get("特情_自比极低阈值%", 30)) / 100.0
    ...
    if pareto == "常规产品":
        ...
        high_growth_thr = float(thr.get("特情_常规高增长阈值", 1.0))
    ...
    # v2.8fix: 近12月零销量 → 所有指标基于零交易，语义受限
    if is_zero_sales:
    ...
    if row.get("_negative_margin"):
        ...
        # v2.9: 补充衰退期中近12月毛利率为负但 _negative_margin 未触发的场景
        notes.insert(0, "当前处于亏损状态（近12月毛利率为负）")
    ...
    # v2.9: 爆发增长后实际下滑 → 提示复核画像（避免健康扩张/成长期+下滑的语义矛盾）
...
def run_profiling(df, latest_month, thr, name_col, date_col, qty_col, rev_col, profit_col, cust_col, order_col, cat_col, ref_priority, wgt, mode='full'):
    """核心画像引擎。返回 (result_df, data_insufficient_list, out, ratio_cols, pp_cols, _t4).

    参数:
        mode='full': 完整计算（含ETS预测、价格弹性、订单频次）
        mode='portrait_only': 仅画像+风险评分（跳过ETS预测），用于历史画像追踪
    """
    _t_func_start = time.time()
    # Winsorization 统计（v2.5）
    # 注：count 包含自然为 0% 的行（发货金额=0）和自然为 75% 的行，
    # 并非全部为被钳制的异常值，仅作数量级参考
    _winsor_low = float(thr.get("Winsor下限", -0.50))
    _winsor_high = float(thr.get("Winsor上限", 0.75))
    n_total_rows = len(df)
    ...
    _t_preloop = time.time(); print(f"  [计时] 窗口划分+预聚合: {_t_preloop - _t_func_start:.1f}s")
    ...
    for _, pinfo in valid_prods.iterrows():
        ...
        row_out["_growth_clamped"] = (growth_raw != growth)  # 标记是否被截断
        ...
        asp_up_threshold = float(thr.get("ASP方向_上升阈值%/月", 0.5)) / 100
        asp_down_threshold = float(thr.get("ASP方向_下降阈值%/月", -0.5)) / 100
        ...
        asp_diag_t = float(thr.get("联合诊断_ASP阈值%/月",
            thr.get("ASP_中分阈值%/月", -0.5))) / 100
        margin_diag_t = float(thr.get("联合诊断_毛利率阈值%/月",
            thr.get("斜率_中分阈值%/月", -0.3))) / 100
        if asp_slope < asp_diag_t and slope_ratio < margin_diag_t:
            row_out["ASP-毛利率联合诊断"] = "价格战风险"
        elif asp_slope > asp_diag_t and slope_ratio < margin_diag_t:
        ...
        # ---- 全量模式：ETS预测 + 价格弹性 + 订单频次（历史画像追踪时跳过） ----
        if mode == 'full':
    ...
    if len(not_new) == 0:
        ...
        # 预计算每组加权均值和产品数（向量化：groupby 替代 iterrows）
    ...
    exit_min_hist = int(thr.get("退市最少历史月数", 3))
    ...
    th_r = float(thr.get("他比严重阈值(pp)", -10))
    ...
    for i, row in result_df.iterrows():
        ...
        # 完整九宫格定位（v2.8fix: 传入row用于上下文感知策略生成）
        ...
        if row["近12月增长率%"] > growth_cap and row["增速衰减(pp)"] < decay_thresh:
            last3_growth_ratio = (row.get("_yoy_change") if pd.notna(row.get("_yoy_change")) else None)
            if last3_growth_ratio is not None and last3_growth_ratio <= 0:
        ...
        t_low  = float(thr.get("总风险_低风险上限", 25))
        t_mid  = float(thr.get("总风险_中风险上限", 50))
        t_high = float(thr.get("总风险_高风险上限", 75))
        if total <= t_low:
...
# ---- 5.18 整理输出列（v2.8: 按逻辑分组排列，内部字段不输出） ----
    output_cols = [
        # ── 基础信息 ──
        "产品名称", "所属参照组", "帕累托分类",
        "最新数据月份", "日历月龄", "活跃月数",
...
def generate_sankey(result_df, hist_intervals, output_dir, base_name):
    """生成产品画像流转桑基图。基于历史画像追踪的时间点显示画像变迁。

    从 hist_intervals 中自动选取最多 5 个等间距时间点，
    仅展示销量前50%的产品以保持图表清晰。
    """
    if not hist_intervals or len(hist_intervals) < 2:
        return

    # 从 hist_intervals 中均匀选取最多 5 个点
    sorted_pts = sorted(hist_intervals)
    if len(sorted_pts) > 5:
        step = (len(sorted_pts) - 1) / 4.0
        selected = [sorted_pts[min(int(round(i * step)), len(sorted_pts)-1)] for i in range(5)]
    else:
        selected = sorted_pts
    # 反转：从远到近（左旧右新），t-12在左，t-1在右
    selected = selected[::-1]

    hist_cols = [f"当前画像_t-{i}" for i in selected if f"当前画像_t-{i}" in result_df.columns]
    if len(hist_cols) < 2:
        return

    # 仅保留销量前100的产品，过滤长尾以保持图表清晰
    sales_col = None
    for c in ["近12月销量", "近12月销售额", "近12月营收"]:
        if c in result_df.columns:
            sales_col = c
            break
    if sales_col is not None:
        df_sorted = result_df.dropna(subset=[sales_col]).sort_values(sales_col, ascending=False)
        top_products = df_sorted.head(100)['产品名称'].unique()
        sankey_df = result_df[result_df['产品名称'].isin(top_products)].copy()
        print(f"  [桑基图] 销量Top100产品: {len(sankey_df)}个 (共{len(result_df)}个)")
    else:
        sankey_df = result_df.copy()

    # 合并小类别，减少节点数
    merge_small = {"新品观察", "清仓/偶发", "未知"}

    portrait_colors = {
        "成长期": "#4CAF50", "健康扩张": "#8BC34A", "利润优化": "#FFC107",
        "现金牛": "#2196F3", "主动收缩": "#FF9800", "夕阳产品": "#9E9E9E",
        "预警增长": "#FF5722", "隐性衰退": "#F44336", "衰退期": "#B71C1C",
        "其他": "#78909C"
    }
    ...
    for col in hist_cols:
        if col in sankey_df.columns:
            col_display = col + '_disp'
            sankey_df[col_display] = sankey_df[col].apply(
                lambda x: "其他" if pd.notna(x) and str(x) in merge_small else
                          ("其他" if pd.isna(x) else str(x))
            )

    hist_cols_disp = [c + '_disp' for c in hist_cols]
    time_labels = [f"t-{i}月" for i in selected]
    ...
    for t_idx, col_disp in enumerate(hist_cols_disp):
        unique_vals = sankey_df[col_disp].dropna().unique()
        for val in unique_vals:
            node_key = f"{val}___{t_idx}"
            if node_key not in label_set:
                label_set[node_key] = len(labels)
                display = f"{val} ({time_labels[t_idx]})"
                labels.append(display)
                node_colors.append(portrait_colors.get(str(val), "#78909C"))

    for i in range(len(hist_cols_disp) - 1):
        col_from = hist_cols_disp[i]
        col_to = hist_cols_disp[i + 1]
        transitions = sankey_df.groupby([col_from, col_to]).size().reset_index(name='count')
        for _, row in transitions.iterrows():
            src_val = str(row[col_from]) if pd.notna(row[col_from]) else "其他"
            tgt_val = str(row[col_to]) if pd.notna(row[col_to]) else "其他"
            src_key = f"{src_val}___{i}"
            tgt_key = f"{tgt_val}___{i + 1}"
            if src_key in label_set and tgt_key in label_set:
                sources.append(label_set[src_key])
                targets.append(label_set[tgt_key])
    ...
    link_colors = []
    for s in sources:
        base = node_colors[s] if s < len(node_colors) else "#78909C"
        base = base.lstrip("#")
    ...
    fig.update_layout(
        title=dict(text="产品画像流转桑基图（季度变迁）", font=dict(size=18, color="#1A3C6E")),
    ...
    sankey_path = os.path.join(output_dir, f"{base_name}_桑基图.html")
    fig.write_html(sankey_path, include_plotlyjs='cdn')
    print(f"  [图表] 桑基图已保存: {sankey_path}")


def generate_portrait_chart(result_df, output_dir, base_name):
    """生成当前画像分布柱状图。"""
    if '当前画像' not in result_df.columns:
        return
    dist = result_df['当前画像'].value_counts().reset_index()
    dist.columns = ['画像', '产品数']
    portrait_order = [
        "成长期", "健康扩张", "利润优化", "现金牛",
        "主动收缩", "夕阳产品",
    ...
    dist = dist.sort_values('_order').reset_index(drop=True)
    
    colors = {
        "成长期": "#4CAF50", "健康扩张": "#8BC34A", "利润优化": "#FFC107",
        "现金牛": "#2196F3", "主动收缩": "#FF9800", "夕阳产品": "#9E9E9E",
        "预警增长": "#FF5722", "隐性衰退": "#F44336", "衰退期": "#B71C1C",
        "新品观察": "#00BCD4", "清仓/偶发": "#607D8B"
    }
    bar_colors = [colors.get(p, "#999999") for p in dist['画像']]
    
    fig = go.Figure(data=[go.Bar(
        x=dist['画像'], y=dist['产品数'],
        marker=dict(color=bar_colors),
        text=dist['产品数'], textposition='auto'
    )])
    fig.update_layout(
        title=dict(text="产品画像分布", font=dict(size=16)),
        xaxis=dict(title="画像", tickangle=45),
        yaxis=dict(title="产品数"),
        height=500, width=900
    )
    chart_path = os.path.join(output_dir, f"{base_name}_画像分布.html")
    fig.write_html(chart_path, include_plotlyjs='cdn')
    print(f"  [图表] 画像分布图已保存: {chart_path}")


def generate_risk_chart(result_df, output_dir, base_name):
    """生成风险等级分布饼图。"""
    if '衰退风险等级' not in result_df.columns:
        return
    dist = result_df['衰退风险等级'].value_counts().reset_index()
    dist.columns = ['等级', '产品数']
    order = ["低风险", "中风险", "高风险", "极高风险"]
    dist['_order'] = dist['等级'].apply(
        lambda x: order.index(x) if x in order else 99
    )
    dist = dist.sort_values('_order').reset_index(drop=True)
    
    fig = go.Figure(data=[go.Pie(
        labels=dist['等级'], values=dist['产品数'],
        marker=dict(colors=["#4CAF50", "#FFC107", "#FF5722", "#B71C1C"]),
        textinfo='label+percent'
    )])
    fig.update_layout(
        title=dict(text="衰退风险等级分布", font=dict(size=16)),
        height=500, width=700
    )
    chart_path = os.path.join(output_dir, f"{base_name}_风险分布.html")
    fig.write_html(chart_path, include_plotlyjs='cdn')
    print(f"  [图表] 风险分布图已保存: {chart_path}")
...
def main():
    ...
    if order_col and order_col in df.columns:
        ...
        print(f"  [v2.8] 订单号列已加载: {order_col}")
    else:
        order_col = None
        print("  [v2.8] 订单号列未配置或不存在，跳过订单频次分析（关联分析改用客户+月份方案，不受影响）")
    ...
    result_df, data_insufficient_list, out, ratio_cols, pp_cols, _t4 = run_profiling(
    ...
    if len(hist_intervals) > 0:
        # v2.8fix: 历史追踪列不再合并到主表，改为独立Sheet输出
        # 构建历史画像追踪DataFrame（产品名称 + 各时间点×6指标）
    ...
    # v2.7: 预先计算RFM和关联分析（v2.8改: 关联分析按客户+月份聚合）
    rfm_df = rfm_customer_segmentation(df, date_col, cust_col, rev_col, thr)
    assoc_df = product_association_analysis(df, name_col, date_col, cust_col, thr)
    forecast_cols_check = ["预测第1月销量", "预测第2月销量", "预测第3月销量", "销量趋势预测"]
    output_path = os.path.join(SCRIPT_DIR, f"output_v2.8_{latest_month}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
    ...
    if data_insufficient_list:
        ...
        ws4.cell(row=1, column=8, value=f"说明：日历月龄 < {int(thr.get('最低记录月数', 3))} 个月的产品，数据不足以参与任何分析。").font = Font(size=10, italic=True, color="666666")
    ...
    # ===== Sheet 9: 历史画像追踪（v2.8fix: 独立Sheet） =====
    if len(hist_intervals) > 0:
    ...
    explanations = [
        ...
        "毛利率同比变化%：近12月毛利率 − 前12月毛利率，单位百分点。正数表示毛利率同比改善。",
        ...
        f"新品观察 — 日历月龄不足{int(thr.get('新品观察月数', 12))}个月或销量不足{float(thr.get('新品观察最低销量', 100))}（可配置）。持续跟踪，暂不判定。",
        "",
        "【管理层摘要列】",
        "投入区   = 成长期 + 健康扩张 → 主动投入资源",
        ...
        "",
        "【数据不足产品清单】",
        f"日历月龄 < {int(thr.get('最低记录月数', 3))} 个月的产品不进入产品快照表，仅在此清单记录。",
        "",
        "【新品判定模式】",
        f"当前模式：{str(thr.get('新品判定模式', '月数')).strip()}。可在 config.xlsx → 阈值参数 → 新品判定模式 中切换。",
        "",
        "【预测字段说明】",
...
# ---- 5.20 输出统计 ----
    ...
    high_risk_threshold = float(thr.get("总风险_中风险上限", 50))
    ...
    print(f"数据不足(<{int(thr.get('最低记录月数', 3))}月): {insuf_count}")
    print(f"进入快照表: {total}")
    ...
    print()
    print("另输出3个HTML可视化图表（桑基图、画像分布图、风险分布图）")
    print()
    ...
    try:
        generate_sankey(result_df, hist_intervals, SCRIPT_DIR, base_name)
    except Exception as e:
        print(f"  [警告] 桑基图生成失败: {e}")
    try:
        generate_portrait_chart(out, SCRIPT_DIR, base_name)
    except Exception as e:
        print(f"  [警告] 画像分布图生成失败: {e}")
    try:
        generate_risk_chart(out, SCRIPT_DIR, base_name)
    except Exception as e:
        print(f"  [警告] 风险分布图生成失败: {e}")
    print(f"  [计时] 图表生成: {time.time() - _t_chart:.1f}s")
