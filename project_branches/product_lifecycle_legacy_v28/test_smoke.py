...
SKIP_REASON = '需 所有的出货明细5.9.xlsx(137MB,未随包;原位置 E:\\3-其他资料\\数据分析\\产品生命周期评估\\),放入本分支根目录后运行'
SAMPLE_GLOBS = ['所有的出货明细5.9.xlsx']      # 样例数据(相对分支根),全部存在才可执行
RUN_CMDS = [('.', ['run_v2.8.py'])]              # 相对分支根的工作目录 + 命令
EXPECTED = {}              # 相对路径 -> {"min_rows": n, "key_cols": [...]}
EXPECTED_GLOBS = ['output_v2.8_*.xlsx']  # 只需存在且非空的文件模式
