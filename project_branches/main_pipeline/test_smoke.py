...
SKIP_REASON = '财务分析 xlsx 未随包(>50MB)。请将任一 财务分析-N月.xlsx 放入 data/ (原位置: E:\\3-其他资料\\数据分析\\工作文件\\semiconductor_analysis\\data\\财务分析-6月（7.6）.xlsx)'
SAMPLE_GLOBS = ['data/*.xlsx']      # 样例数据(相对分支根),全部存在才可执行
RUN_CMDS = [('.', ['run_all.py', '--stage', 'silver'])]              # 相对分支根的工作目录 + 命令
EXPECTED = {'output/silver/silver_cleaned_rows.csv': {'min_rows': 1000, 'key_cols': ['客户编号']}, 'output/silver/silver_customer_monthly.csv': {'min_rows': 10, 'key_cols': ['客户编号']}, 'output/silver/silver_product_monthly.csv': {'min_rows': 10, 'key_cols': []}, 'output/silver/silver_customer_x_product.csv': {'min_rows': 10, 'key_cols': []}}              # 相对路径 -> {"min_rows": n, "key_cols": [...]}
EXPECTED_GLOBS = []  # 只需存在且非空的文件模式
