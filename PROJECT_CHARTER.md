...
| S10 | **改码收工必须 publish 或登记暂缓**——任何 `sales_analytics_platform\` 代码/配置变更合入 master 后，须立即执行 `kanban\tools\publish_to_share.ps1` 完成共享盘同步；因故暂缓时，须在 `project_analysis\施工进度台账.md` 登记"未同步原因"与预计补同步时间；发布路径以 `kanban\tools\publish_config.json` 为单点配置，发布后须通过 `code\version.txt` 哈希自检 | 台账记录 / publish 退出码 0 且 version.txt 回读一致 |
