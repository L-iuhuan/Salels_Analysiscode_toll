...
PRODUCT_LIFECYCLE = {
    ...
    # ---- 口径开关（批次① T1 预留，默认关闭；待批次③业务拍板 D-1/D-2 后按需启用）----
    # 产品管道独立入口（product_lifecycle/run.py::_prepare_data）的历史 v2.8 行为：
    #   - _口径_负销量过滤: True=剔除负销量/零销量（退货/红冲/空单）；
    #       默认 False=与生产链式路径一致（保留负销量，聚合时自然冲减）
    #   - _口径_毛利率钳制: True=对毛利率 clip(Winsor下限, Winsor上限) 并据此裁剪利润；
    #       默认 False=与生产链式路径一致（保留真实毛利率，_利润_裁剪=原始利润）
    # 当前两条运行路径（链式+独立）均已统一为"不过滤、不钳制"，见 run.py::run/_prepare_data。
    "_口径_负销量过滤": False,
    "_口径_毛利率钳制": False,
