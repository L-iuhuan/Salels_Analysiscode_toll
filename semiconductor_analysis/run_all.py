...
def stage_silver(source_path: str) -> tuple:
    ...
    # 保留原始列用于渠道推导（_derive_channel需要"代理商/直供名称"原始列名）
    _channel_cols = ["代理商/直供名称", "实际终端客户", "销售部门", "实际业务员"]
    raw_for_channel = raw[[c for c in _channel_cols if c in raw.columns]].copy()
    ...
    # 验证必需列
    required = [
        COL_MAP.get("customer_id", "客户编号"),
        COL_MAP.get("product_name", "产品品种"),
        COL_MAP.get("quantity", "数量"),
        COL_MAP.get("revenue", "金额"),
        COL_MAP.get("ship_date", "发货日期"),
    ]
    missing = validate_required_columns(raw, required, context="stage_silver")
    if missing:
        print(f"  [错误] 缺少必需列: {missing}，管道无法继续")
        print(f"  当前数据列: {list(raw.columns)}")
        return False, None, None

    raw = filter_negative_qty(raw, qty_col=COL_MAP.get("quantity", "数量"))
    raw = winsorize_margins(raw)
    _validator.validate_clean(raw)  # V2: 清洗后验证
    ...
    _validator.validate_silver(silver)  # V3: Silver层验证

    # 产品线列传递：将产品一级分类带入 customer_x_product（用于品类分析）
    cat_col = "产品一级分类"
    ...
    # 将渠道推导原始列合并回返回的raw_data（portrait.py的_derive_channel需要原始列名）
    for _col in raw_for_channel.columns:
        if _col not in raw.columns:
            raw[_col] = raw_for_channel[_col]
