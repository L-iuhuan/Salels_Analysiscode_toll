...
def calc_category_migration(
    ...
    # 计算上期品类占比（对比变化）
    result = result.sort_values(["客户编号", "品类", "期间"], kind='stable')
    for col in ["品类占比"]:
        result[f"{col}变化"] = result.groupby(["客户编号", "品类"])[col].diff()

    # 品类排名（每期间内）
    result["品类排名"] = result.groupby(["客户编号", "期间"])["品类收入"].rank(
        method="dense", ascending=False
    ).astype(int)

    # 排名变化
    result["排名变化"] = result.groupby(["客户编号", "品类"])["品类排名"].diff()
