...
def estimate_true_profit(
    ...
    if revenue <= 0:
        ...
        # 如无退货金额，按收入估算 2% 退货率
        default_return_rate = float(cfg.get("default_return_rate", 0.02))
        if return_amount <= 0:
            aftersales_cost = revenue * default_return_rate * aftersales_ratio

        # 4. 资金占用成本（无应收账款数据时按营收×DSO/365估算）
        if receivable <= 0:
            # 估算应收 ≈ 营收 × DSO / 365（即账期内未回款的营收）
            receivable = revenue * dso / 365.0
    ...
    # 利润等级（阈值从配置读取）
    margin_tiers = cfg.get("profit_margin_tiers", {"high_profit_pct": 15, "low_profit_pct": 0})
    _high_pct = margin_tiers.get("high_profit_pct", 15)
