...
methods_csv_path = os.path.join(FORECAST_DIR, "候选预测方法清单.csv")
if not os.path.exists(methods_csv_path):
    raise FileNotFoundError(f"Required file not found: {methods_csv_path}")
methods = pd.read_csv(methods_csv_path)
rank_csv_path = os.path.join(FORECAST_DIR, "预测方法排行榜.csv")
if not os.path.exists(rank_csv_path):
    raise FileNotFoundError(f"Required file not found: {rank_csv_path}")
ranking = pd.read_csv(rank_csv_path)
...
def extract_param(pstr, key):
    ...
    try:
        params = ast.literal_eval(pstr) if isinstance(pstr, str) else pstr
...
methods.loc[large_win, '筛选说明'] = methods.loc[large_win, '窗口'].apply(lambda w: f"窗口={int(w)}，需在回测中记录每折窗口一致性")
