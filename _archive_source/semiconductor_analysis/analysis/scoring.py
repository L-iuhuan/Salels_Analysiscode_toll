...
def calc_composite_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ---- 活跃客户标记（窗口内有交易的客户） ----
    active = (
        result["近12月收入"].fillna(0) > 0
        if "近12月收入" in result.columns
    ...
    if "订单处理成本" in result.columns and "近12月收入" in result.columns:
        ...
        result["物流成本率"] = (result["物流成本"] / rev_safe * 100).fillna(0)
...
def _classify_dual_axis(row, value_threshold=None, growth_threshold=None) -> str:
    """依据价值贡献分和增长动能分进行双轴分类。"""
