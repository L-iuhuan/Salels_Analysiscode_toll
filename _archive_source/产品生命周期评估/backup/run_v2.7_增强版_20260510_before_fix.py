...
def run_profiling(df, latest_month, thr, name_col, date_col, qty_col, rev_col, profit_col, cust_col, order_col, cat_col, ref_priority, wgt):
    ...
    min_history_months = int(thr.get("新品观察月数", 12))
...
def main():
    ...
    for offset_months in hist_intervals:
        ...
        print(f"  [历史追踪] t-{offset_months}月 ({tp}): {len(hist_result)}个产品")
    ...
    explanations = [
        ...
        "  例如：若ASP数据不足（因子6不可靠），则因子6权重=0，其余因子权重重新计算：",
        "    原权重：20%(斜率) + 15%(客户) + 15%(CV) + 25%(衰减) + 5%(历史) + 20%(ASP)",
        "    归一化后：25%(斜率) + 18.75%(客户) + 18.75%(CV) + 31.25%(衰减) + 6.25%(历史) + 0%(ASP)",
