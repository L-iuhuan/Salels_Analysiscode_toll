...
【输出文件】output_v2.8_年月.xlsx
  产品快照表: 每个产品一行，完整诊断字段（含趋势预测等指标）
  预警清单: 自动筛选高风险产品
  画像分布: 各画像产品数统计
  趋势预测汇总: 未来3月销量预测（ETS+节假日调整）
  使用说明: 字段解释与术语表
...
def product_association_analysis(df, order_col, name_col, thr):
    """产品关联分析（购物篮分析）。返回含支持度/置信度/提升度/杠杆率/确信度的DataFrame。"""
    ...
    for (p1, p2), count in pair_counts.items():
        ...
        sup_p1 = product_counts[p1] / total_orders
        sup_p2 = product_counts[p2] / total_orders
...
def run_profiling(df, latest_month, thr, name_col, date_col, qty_col, rev_col, profit_col, cust_col, order_col, cat_col, ref_priority, wgt):
...
# ---- 5.18 整理输出列（v2.8: 按逻辑分组排列，内部字段不输出） ----
    output_cols = [
        ...
        # ── 策略与说明 ──
        "通用策略建议", "特情说明",
        # 注：_w_斜率、_w_客户、_w_波动、_w_衰减、_w_历史、_w_ASP、_w_sum 等内部字段
        # 以及 _slope_data_insufficient、_zero_profit、_cust_missing、_cv_invalid、_hist_insufficient、_asp_insufficient
        # 仅为代码内部计算使用，不输出到Excel
    ]
...
def generate_sankey(result_df, hist_intervals, output_dir, base_name):
    """生成产品画像流转桑基图。按季度时间点显示画像变迁。"""
    if not hist_intervals or len(hist_intervals) < 2:
        return
    
    q_points = [3, 6, 9, 12]
    hist_cols = [f"当前画像_t-{i}" for i in q_points if f"当前画像_t-{i}" in result_df.columns]
    if len(hist_cols) < 2:
        return
    ...
    portrait_colors = {
        ...
        "新品观察": "#00BCD4", "清仓/偶发": "#607D8B", "未知": "#CCCCCC"
    }
    
    time_labels = [f"t-{i}月" for i in q_points if f"当前画像_t-{i}" in result_df.columns]
    ...
    for t_idx, col in enumerate(hist_cols):
        ...
        for val in unique_vals:
            ...
            if node_key not in label_set:
                ...
                node_colors.append(portrait_colors.get(str(val), "#CCCCCC"))
    
    for i in range(len(hist_cols) - 1):
        col_from = hist_cols[i]
        col_to = hist_cols[i + 1]
        transitions = result_df.groupby([col_from, col_to]).size().reset_index(name='count')
        for _, row in transitions.iterrows():
            src_val = str(row[col_from]) if pd.notna(row[col_from]) else "未知"
            tgt_val = str(row[col_to]) if pd.notna(row[col_to]) else "未知"
    ...
    link_colors = []
    for s in sources:
        base = node_colors[s] if s < len(node_colors) else "#CCCCCC"
        link_colors.append(base.replace("0x", "").replace("#", "") + "66")
...
def main():
    ...
    if order_col and order_col in df.columns:
        ...
        order_col = None
        print("  [v2.8] 订单号列未配置或不存在，跳过订单频次和关联分析")
    ...
    # ---- v2.8: 历史画像滑动窗口追踪 ----
    _v = thr.get("历史画像_启用", 0)
    hist_enabled = 1 if str(_v).strip().upper() in ("TRUE", "1", "YES") else 0
    if hist_enabled != 1:
        hist_intervals = []
    else:
    ...
    if assoc_df is not None and len(assoc_df) > 0:
        ...
        assoc_cols = ['产品A', '产品B', '支持度', '置信度(A->B)', '提升度(A->B)', '杠杆率(A->B)', '确信度(A->B)', '共现订单数']
        ...
        print("  [v2.8] 订单数据不足或无订单号，跳过产品关联分析")
...
# ---- 5.20 输出统计 ----
    ...
    print("  使用说明         - 字段解释、名词定义与术语表（含v2.8新字段）")
    print()
