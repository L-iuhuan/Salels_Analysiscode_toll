...
SAMPLE_GLOBS = ['output/quarterly_forecast_customer/预测方案总行版.csv']      # 样例数据(相对分支根),全部存在才可执行
RUN_CMDS = [('.', ['unified_forecast_v3.py'])]              # 相对分支根的工作目录 + 命令
EXPECTED = {}              # 相对路径 -> {"min_rows": n, "key_cols": [...]}
EXPECTED_GLOBS = ['*.csv']  # 只需存在且非空的文件模式
