...
def generate(excel_path, output_path=None):
    ...
    if not template_path.exists():
        print(f"错误：找不到模板文件 template.html")
        sys.exit(1)
    ...
    if output_path is None:
        output_path = excel_path.parent / f"{stem}_v4.html"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # 自动查找最新的 output_v2.8_*.xlsx
        excel_files = sorted(SCRIPT_DIR.glob("output_v2.8_*.xlsx"))
        if excel_files:
