...
【配置文件】同级目录下的 config.xlsx
  ...
  风险因子权重 Sheet → 调整6个风险因子的权重（合计须=1.0）
...
def generate_specific_note(row, thr):
    ...
    if row.get("近12月销量", 0) == 0 and row.get("当前画像") not in ["新品观察", "清仓/偶发", None]:
    ...
    if row.get("_growth_clamped"):
        ...
        if g_raw >= 5.0:
            notes.append("增长率已达截断上限(500%)，实际增速更高")
        elif g_raw <= -1.0:
            notes.append("增长率已达截断下限(-100%)，实际跌幅更深")

    # v2.8fix: 爆发增长后实际下滑（decay cap被移除）
    ...
    if row.get("当前画像") == "衰退期" and margin_yoy > 5:
...
def run_profiling(df, latest_month, thr, name_col, date_col, qty_col, rev_col, profit_col, cust_col, order_col, cat_col, ref_priority, wgt, mode='full'):
    ...
    for i, row in result_df.iterrows():
        ...
        # v2.8改进：仅当近3月增长率仍为正（仍在增长，只是增速放缓）时才cap；
        #          若近3月增长率已转负（实际下滑），取消cap，使用真实风险分数
        ...
        if row.get("_cust_missing"):
            quality_flags.append("CM")
        if row.get("_hist_insufficient"):
            quality_flags.append("HL")
...
def main():
    ...
    explanations = [
        ...
        "【风险评分说明】",
        "6 因子加权评分（0~100分）：",
        "  ① 毛利率趋势斜率（20%）：近12月毛利率线性回归斜率",
        "  ② 客户集中度（15%）：前1大/前3大客户营收占比",
        "  ③ 订货波动性 CV（15%）：月销量变异系数",
        "  ④ 增速衰减（25%）：近3月增长率 vs 近12月增长率 + 近3月同比下降",
        "  ⑤ 同类历史对照（5%）：产品月龄 vs 同类退市产品寿命中位数",
        "  ⑥ ASP趋势（20%）：近12月均价变化斜率，与毛利率趋势联合判定",
        "得分映射：0~30 低风险 | 30~60 中风险 | 60~80 高风险 | >80 极高风险",
        f"因子5兜底：同类退市产品不足{int(thr.get('参照组最低产品数', 3))}个时取 {int(thr.get('同类_品类兜底分',50))} 分（可配置）。",
        ...
        "CM=客户数据缺失 HL=同类历史参照不足 AS=ASP数据不足 GC=增长率被截断 ZS=近12月销量为零",
        "NH=历史无有效毛利率数据 无标记=全部可靠",
        "",
        "【v2.8fix 策略一致性改进】",
        "通用策略建议现已根据实际数据动态调整：",
