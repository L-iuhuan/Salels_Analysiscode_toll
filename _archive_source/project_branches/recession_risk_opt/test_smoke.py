...
SAMPLE_GLOBS = ['output/silver/silver_product_monthly.csv', 'output/silver/silver_customer_x_product.csv', 'recession_risk_opt/data/samples.pkl']      # 样例数据(相对分支根),全部存在才可执行
RUN_CMDS = [('.', ['recession_risk_opt\\phaseB1a_severity_regression.py'])]              # 相对分支根的工作目录 + 命令
EXPECTED = {'recession_risk_opt/output/phaseB1a_results.pkl': {}}              # 相对路径 -> {"min_rows": n, "key_cols": [...]}
EXPECTED_GLOBS = ['recession_risk_opt/output/phaseB1a_*.png', 'recession_risk_opt/output/phaseB1a_*.md']  # 只需存在且非空的文件模式
