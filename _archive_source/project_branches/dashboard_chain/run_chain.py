...
def ensure_output_junction():
    """把 processing/output 指向包根 output/（目录联接，无需管理员权限）。

    前段各模块的输出路径写法不统一：一部分读 config（已指向包根 output/），
    另一部分在各自文件里写死为 processing/output。若不做统一，前段产出会被
    拆到两个地方，后段（读包根 output/）就会读不到。
    用目录联接让 processing/output 成为包根 output/ 的“别名”，两处写法最终
    都落到同一个真实目录 output/，前段代码零改动。每次运行自动校准，可移植。
    """
    link = os.path.join(DIR_PROC, "output")
    target = DIR_OUT
    os.makedirs(target, exist_ok=True)

    # 已是正确的联接 → 直接用
    if (os.path.isjunction(link) or os.path.islink(link)) and \
       os.path.realpath(link) == os.path.realpath(target):
        return
    # 存在但不对 → 移除（联接只删链接本身；真实目录是可再生成的旧产出）
    ...
    print(f"  已联接: processing\\output  →  output\\  (前段产出统一落到单一输出目录)")
