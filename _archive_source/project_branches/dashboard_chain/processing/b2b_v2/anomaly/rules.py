...
def detect_revenue_cliff(customer_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in customer_df.iterrows():
        growth = _safe_val(row, "收入增长率", 0)
        decline_months = _safe_val(row, "连续下滑月数", 0) if has_decline else 0
        ...
        # 规则: 收入增长率 <= -50% 且连续下滑 >= 2个月
        if growth <= -0.5 and decline_months >= 2:
            level = "高" if growth <= -0.8 else "中"
            results.append({
                "客户编号": row["客户编号"],
                "异常类型": "营收断崖",
                "异常等级": level,
