...
def _dim_base_info(cid, cust_info, raw_data=None, attr_map=None):
    """基本信息：渠道类型、客户等级、客户类别、所属区域、业务负责人。
...
def _dim_product_coverage(cp_data):
    ...
    if len(cp_data) > 0:
        top_products = (
            cp_data.groupby("产品品种", observed=True)["rev_sum"].sum().sort_values(ascending=False, kind='stable')
        )
...
def _dim_product_line_distribution(cp_data, cat_col, cat_detail_col=None):
    """产品线分布：产品线数、主导产品线及占比、产品线HHI。

    Phase 0 fix: 新增"实际品类数"从cat_detail_col聚合（"产品一级分类"恒=12无区分度）。
    """
    row = {}
    # 实际品类数（细粒度，从"型号_产品品类"聚合）
    detail_line = cat_detail_col or cat_col
    if detail_line in cp_data.columns:
        detail_rev = cp_data.groupby(detail_line, observed=True)["rev_sum"].sum()
        row["实际品类数"] = len(detail_rev)
    else:
        row["实际品类数"] = 0

    if cat_col in cp_data.columns:
        line_rev = cp_data.groupby(cat_col, observed=True)["rev_sum"].sum()
...
def calc_customer_portrait(silver: dict, source_path: str, latest_month,
    ...
    for cid in customers:
        ...
        row.update(_dim_product_line_distribution(cp_data, cat_col, cat_detail_col))
