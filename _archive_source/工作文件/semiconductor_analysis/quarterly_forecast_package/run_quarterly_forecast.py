...
CRITICAL_COLS = ["发货日期", "型号_产品线（新）", "存货名称", "发货数量", "RMB 未税金额小计", "成本", "利润", "代理商/直供名称", "ERP订单号"]
EPS = 1e-9
...
def clean_and_map(df: pd.DataFrame, log: OperationLog) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for c in ["型号_产品线（新）", "存货名称", "代理商/直供名称", "ERP订单号", "产品线", "产品系列", "型号", "存货编码"]:
        if c in df.columns:
...
def aggregate_layers(df_hist: pd.DataFrame, bucket_rows: List[Dict[str, object]], log: OperationLog):
    ...
    product_bucket = agg(["型号_产品线（新）", "存货名称", "桶编号"])
    pc_bucket = agg(["型号_产品线（新）", "存货名称", "代理商/直供名称", "桶编号"])

    log.add("04聚合", "生成产品线/产品/产品×客户历史期聚合", f"产品线桶={len(line_bucket)}，产品桶={len(product_bucket)}，产品客户桶={len(pc_bucket)}")
...
def recent_product_prices(product_panel: pd.DataFrame, product_lines: pd.DataFrame, bucket_ids: List[str], upto_idx: int) -> pd.DataFrame:
    """按训练截止桶计算产品级最近加权单价/成本单价，带回退。"""
    train_buckets = bucket_ids[:upto_idx]
    rows = []
    keys = product_panel[["型号_产品线（新）", "存货名称"]].drop_duplicates()
    latest_bucket = train_buckets[-1]
    for _, r in keys.iterrows():
        line = r["型号_产品线（新）"]
        prod = r["存货名称"]
        sub = product_panel[(product_panel["型号_产品线（新）"] == line) & (product_panel["存货名称"] == prod) & (product_panel["桶编号"].isin(train_buckets))]
        price = cost = np.nan
        source = "无"
        ...
        if not np.isfinite(price) or not np.isfinite(cost):
            ...
            source = "全局最近4桶回退"
        rows.append({"型号_产品线（新）": line, "存货名称": prod, "销售单价": price, "成本单价": cost, "单价来源": source, "训练截止桶": latest_bucket})
...
def latest_mix(line_product_panel: pd.DataFrame, line: str, train_buckets: List[str], horizon: int, line_qty_pred: np.ndarray, prices: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """产品线级销量预测按最近产品销售结构分摊，再用SKU单价计算金额/成本。"""
    ...
    mix = sub.groupby("存货名称")["销售量"].sum()
    if mix.sum() <= 0:
        mix = pd.Series(1.0, index=sub["存货名称"].drop_duplicates())
    mix = mix / mix.sum()
    pmap = prices[prices["型号_产品线（新）"] == line].set_index("存货名称")[["销售单价", "成本单价"]]
    avg_price = 0.0
    avg_cost = 0.0
    for prod, share in mix.items():
        if prod in pmap.index:
            avg_price += share * float(pmap.loc[prod, "销售单价"])
            avg_cost += share * float(pmap.loc[prod, "成本单价"])
...
def prepare_runtime_context(
    ...
    for (line, prod), sub in product_panel.groupby(["型号_产品线（新）", "存货名称"], dropna=False):
        s = sub.set_index("桶编号").reindex(bucket_ids).fillna(0)
    ...
    for (line, prod, cust), sub in pc_panel.groupby(["型号_产品线（新）", "存货名称", "代理商/直供名称"], dropna=False):
        s = sub.set_index("桶编号").reindex(bucket_ids).fillna(0)
...
def build_product_price_contrib(product_panel: pd.DataFrame, line_panel: pd.DataFrame, bucket_ids: List[str], log: OperationLog) -> pd.DataFrame:
    ...
    contrib = product_panel.merge(prices, on=["型号_产品线（新）", "存货名称"], how="left")
    ...
    log.add("07贡献", "生成产品级价格与历史贡献表", f"行数={len(contrib)}", 行数=len(contrib))
    return contrib
...
def run(
    ...
    product_panel = complete_panel(product_bucket, ["型号_产品线（新）", "存货名称"], bucket_ids, value_cols)
    pc_panel = complete_panel(pc_bucket, ["型号_产品线（新）", "存货名称", "代理商/直供名称"], bucket_ids, value_cols)
